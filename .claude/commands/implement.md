...
Step 4: Validate Specs ✋ 最後確認點
    ...
    ✓ API 規格檢查（完整性、明確性、可測試性）
    ✓ DB Schema 檢查（完整性、效能、資料完整性）
    ✓ 量化指標檢查（效能、品質、安全標準）
    ✓ 一致性檢查（API vs DB, 業務規則 vs 實作）
    ✋ 用戶確認是否啟動 AUTO-RUN
...
#### 4.1 API 規格檢查（參考 spec-validation.md）

**完整性**:
- [ ] 所有 API 端點都有完整文件
- [ ] Request Body 所有欄位有型別、必填、驗證規則
- [ ] Response 有成功（2xx）和錯誤（4xx/5xx）範例
- [ ] HTTP 方法、狀態碼、認證要求明確
- [ ] 錯誤回應有 error code、message、details

**明確性**:
- [ ] Request/Response 範例可直接複製使用
- [ ] 驗證規則可直接轉為 FormRequest
- [ ] 錯誤訊息具體且可操作
- [ ] 無模糊詞（「快」、「好」、「充分」等）
...
#### 4.2 DB Schema 檢查（參考 spec-validation.md）

**完整性**:
- [ ] 所有資料表有欄位定義（名稱、型別、約束）
- [ ] 所有關聯關係已定義（1:1, 1:N, N:N）
- [ ] 所有索引已規劃（PRIMARY, UNIQUE, INDEX, FOREIGN）
- [ ] Migration 程式碼完整（up() 和 down()）
...
**資料完整性**:
- [ ] 主鍵定義
- [ ] 外鍵約束（CASCADE, RESTRICT）
- [ ] NOT NULL 約束
- [ ] UNIQUE 約束
- [ ] DEFAULT 值定義

#### 4.3 量化指標檢查（參考 metrics-standards.md）

**效能標準**:
- [ ] API 回應時間已定義（P50/P95/P99）
- [ ] 資料庫查詢效能已定義（< Xms）
- [ ] 並發處理能力已定義（>= X req/s）

**品質標準**:
- [ ] 測試覆蓋率目標已定義（Feature >= 95%）
...
**API vs DB**:
- [ ] API Request 欄位在 DB Schema 中有對應
- [ ] API Response 資料來源於 DB Schema
- [ ] 驗證規則與 DB 約束一致
